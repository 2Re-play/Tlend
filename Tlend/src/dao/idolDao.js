...
exports.getSupportDefault = (Transaction, req, next) => {
  ...
  return Transaction(async (connection) => {
    console.log()
    const Query1 = `SELECT support_title, support_description, support_currentMoney, support_goalMoney, support_finishDate, support_startDate, support_schedule FROM SUPPORT WHERE support_idx = ${req.params.support_idx}`
